...
export const addRole = (startCli: () => void): void => {
  getDepartments().then((departments) => {
    inquirer
      ...
      .then((answer) => {
        pool.query(
          `SELECT id FROM department WHERE department_name = $1`,
          [answer.department],
          (err: Error, result: QueryResult) => {
            if (err) {
...
const getRoles = async (): Promise<string[]> => {
  try {
    const result: QueryResult = await pool.query(`SELECT title FROM roles`);
